...
class AdvancedPageTurnState extends State<AdvancedPageTurn> with TickerProviderStateMixin {
  ...
  void _turnPage(DragUpdateDetails details, BoxConstraints dimens) {
    ...
    if (_isForward ?? false || pageNumber == 0) {
      ...
      print('if ${_ratio} ${_controllers[pageNumber].value}');
    } else {
      if( pageNumber!=0){
        _controllers[pageNumber - 1].value += _ratio+0.01;
        print('else ${_ratio} ${_controllers[pageNumber].value}');
      }
    }
  }
      ...
      child: LayoutBuilder(
        builder: (context, dimens) => GestureDetector(
          ...
          onHorizontalDragUpdate: (details) {
            print("page turned $pageNumber ${details.delta.dx}");
