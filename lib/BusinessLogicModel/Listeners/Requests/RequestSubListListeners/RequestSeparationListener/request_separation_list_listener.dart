...
import 'package:peopleqlik_debug/BusinessLogicModel/Models/PaysSlipApprovalsRequest/RequestsModel/get_request_separation_list_model.dart';
...
class GetSeparationRequestListListener extends GetChangeNotifier
{
  List<GetSeparationDataList>? requestDataList;
  ...
  async {
    ...
    ApiResponse apiResponse = await GetApisUrlCaller().getSeparationListApiCall(context,'?PageNo=$page&PerPage=10&CompanyCode=${employeeInfoMapper.companyCode}&EmployeeCode=${employeeInfoMapper.employeeCode}');
  ...
  void updateStep(bool event,BuildContext context)
  {
    ...
    if(reachedEnd==false&&event==true)
    {
      ...
      requestDataList?.add(GetSeparationDataList());
      reachedEndList(true);
