...
import 'package:peopleqlik_debug/BusinessLogicModel/ApiCalls/post_request_encashment_submit_api.dart';
...
import 'package:peopleqlik_debug/BusinessLogicModel/Models/PaysSlipApprovalsRequest/RequestsModel/post_request_encashment_form_mapper.dart';
...
class RequestSpecialFormListener extends GetChangeNotifier with GetLoader
{
  ...
  async {
    ...
    ApiResponse? apiResponse = await GetApisUrlCaller().getEncashmentRequestFormApiCall(context,'?CompanyCode=${employeeInfoMapper.companyCode}&EmployeeCode=${employeeInfoMapper.employeeCode}');
    ...
    )
      {
        ...
        try{
          if(saveSpecialRequestFormMapper?.encashmentUnit!=null&&saveSpecialRequestFormMapper!.encashmentUnit!.isNotEmpty&&double.parse(saveSpecialRequestFormMapper!.encashmentUnit!)<=specialBalanceListener.specialRequestBalanceResultSet?[0].maxEncashmentUnit&&double.parse(saveSpecialRequestFormMapper!.encashmentUnit!)<=specialBalanceListener.specialRequestBalanceResultSet?[0].balance)
          {
            ...
            initLoader();
            ApiResponse? apiResponse = await GetApisUrlCaller().saveEncashmentRequestApi(context,list);
            await closeLoader();
