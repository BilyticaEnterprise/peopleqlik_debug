...
class RequestListSubPage extends StatelessWidget {
  RequestDataTaker? requestDataTaker;
  RequestListSubPage({Key? key}) : super(key: key);
  @override
                ...
                selectEmployeeTap: () {
                  EmployeeSearchBottomSheet().show(
                      context,
                          (employeeInfoMapper)
                      {
                        Provider.of<GetRequestListListener>(context,listen: false).start(context,ApiStatus.started,requestDataTaker!.id);
                      }
                  );
                },
...
class _BodyDataState extends State<BodyData> {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      Provider.of<GetRequestListListener>(context,listen: false).start(context,ApiStatus.started,widget.requestDataTaker!.id);
    });
  ...
  @override
  Widget build(BuildContext context) {
    return Consumer<GetRequestListListener>(
            ...
            return NotificationListener<ScrollNotification>(
                onNotification: (scrollNotification){
                  ...
                  if(scrollNotification.metrics.pixels>0.0&&scrollNotification.metrics.pixels >= scrollNotification.metrics.maxScrollExtent-20)
                  {
                    ...
                    getRequestListListener.updateStep(true, context,widget.requestDataTaker!.id);
                    return true;
                  }
                  else
                  {
                    getRequestListListener.updateStep(false, context,widget.requestDataTaker!.id);
                    return false;
                  }
                },
...
class RequestListWidget extends StatelessWidget {
  final int? index;
  final RequestListDataList? requestListDataList;
  final RequestDataTaker? requestDataTaker;
  const RequestListWidget(this.requestListDataList,this.index, this.requestDataTaker,{Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Provider.of<GetRequestListListener>(context,listen: false).reachedEnd&&index==Provider.of<GetRequestListListener>(context,listen: false).requestDataList!.length-1?
    Container(
      ...
      child: ClipRRect(
        ...
        child: Material(
          ...
          child: InkWell(
            ...
            onTap: (){
              Navigator.pushNamed(context, CurrentPage.RequestDetailPage,arguments: AllRequestDetailMapper(documentNumber: requestListDataList?.documentNo,companyCode: requestListDataList?.companyCode.toString(),screenID: requestListDataList?.screenID.toString(),isApprovalScreen: false));
            },
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Flexible(
                    child: Text(
                      '${requestListDataList?.employeeName}',
                      style: GetFont.get(
                          context,
                          fontSize: 1.8,
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: Text(
                          '${GetDateFormats().getFilterDate(requestListDataList?.requestDate)}',
                          style: GetFont.get(
                              context,
                              fontSize: 1.4,
                              fontWeight: FontWeight.w400,
...
class RequestDetailData
{
  dynamic requestCode;
  dynamic managerId;
  RequestDetailData(this.requestCode,this.managerId);
}