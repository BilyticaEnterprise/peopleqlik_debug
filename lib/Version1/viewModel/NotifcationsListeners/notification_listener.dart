...
class GetNotificationModelListener extends GetChangeNotifier with GetPaginationClasses
{
  ...
  passInitialData() {
    ...
    dataList ??= List<NotificationResultSet>.empty(growable: true); /// If list null then create list
    receiveInitialData(dataList,NotificationResultSet()); /// pass this list to Pagination Mixin class
  }
  ...
  async {
    ...
    ApiResponse? apiResponse = await UseCaseGetApisUrlCaller().getNotificationList(context,'?PageNo=$page&PerPage=10');
    if(apiResponse.apiStatus == ApiStatus.done)
    {
      ...
      addAllList(apiResponse.data!.resultSet); /// New data is inserting in list
      notifyListeners();
